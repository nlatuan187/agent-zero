        ...
        while True:
            try:
                ...
                while True:
                    ...
                    # Safety check: prevent infinite loops with self-healing
                    if self.loop_data.iteration > MAX_ITERATIONS + 5:
                        error_msg = f"Agent exceeded maximum iteration limit ({MAX_ITERATIONS + 5}) even after guidance. Forcing stop to prevent infinite loop. Please refine your request."
                        PrintStyle(font_color="red", bold=True).print(error_msg)
                        self.context.log.log(type="error", content=error_msg)
                        self.hist_add_warning(message=error_msg)
                        
                        # Return a final response to break the outer loop and return control to user
                        from python.helpers.tool import Response
                        return Response(message=error_msg, break_loop=True)

                    elif self.loop_data.iteration == MAX_ITERATIONS:
                        warning_msg = f"Agent has reached {MAX_ITERATIONS} iterations. Requesting guidance from Utility Agent..."
                        PrintStyle(font_color="orange", bold=True).print(warning_msg)
                        self.context.log.log(type="warning", content=warning_msg)
                        
                        # Self-Healing: Ask Utility Agent for guidance
                        try:
                            # Verify utility model is available
                            if self.config.utility_model:
                                system_prompt = "You are a suvervisor for an AI agent. The agent is stuck in a loop. Analyze the recent history and provide a short, stern instruction to stop the repetitive action and try a different approach. Do not be polite. Be direct."
                                # Get recent history (last 5 messages)
                                history_context = self.history.serialize()[-5:] 
                                guidance = await self.call_utility_model(system=system_prompt, message=str(history_context))
                                
                                guidance_msg = f"SUPERVISOR INTERVENTION: {guidance}"
                                PrintStyle(font_color="magenta", bold=True).print(guidance_msg)
                                self.hist_add_warning(message=guidance_msg)
                            else:
                                PrintStyle(font_color="red").print("Utility model not configured, skipping self-healing.")
                        except Exception as e:
                            PrintStyle(font_color="red").print(f"Self-healing failed: {e}")
