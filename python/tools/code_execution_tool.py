...
class CodeExecution(Tool):
    ...
    async def get_terminal_output(
        ...
        while True:
            ...
            if now - start_time > max_exec_timeout:
                ...
                # Force reset session to kill orphan processes
                PrintStyle.warning(f"Session {session} timed out. Resetting to kill process...")
                await self.prepare_state(reset=True, session=session)
    ...
    def fix_full_output(self, output: str):
        ...
        output = truncate_text_agent(agent=self.agent, output=output, threshold=50000) # ~50KB, larger outputs should be dumped to file, not read from terminal
        return output
